...
function main() {
  ...
  program
    ...
    .option('-d, --daemon', 'Run mcc node as daemon')
  ...
  var pidFile = path.join(baseDir, 'mcc.pid');
  if (fs.existsSync(pidFile)) {
    console.log('Failed: mcc server already started');
    return;
  }
  ...
  if (appConfig.netVersion === 'mainnet') {
    ...
    for (var i = 0; i < seeds.length; ++i) {
      appConfig.peers.list.push({ ip: ip.fromLong(seeds[i]), port: 16833 });
    }
  }
  ...
  if (program.daemon) {
    console.log('MCC server started as daemon ...');
    require('daemon')();
