    ...
    connect: ['config', 'genesisblock', 'logger', 'network', function (cb, scope) {
      ...
      scope.network.app.use(queryParser({
        parser: function (value, radix, name) {
          ...
          // console.log("after " + parseInt(value));
          // console.log("equal " + (value == String(parseInt(value))));

          if (isNaN(value) ||  String(parseInt(value)) != value || isNaN(parseInt(value, radix))) {
            return value;
          }
      ...
      scope.network.server.listen(scope.config.port, scope.config.address, function (err) {
        scope.logger.log("MCC started: " + scope.config.address + ":" + scope.config.port);

        if (!err) {
          if (scope.config.ssl.enabled) {
            scope.network.https.listen(scope.config.ssl.options.port, scope.config.ssl.options.address, function (err) {
              scope.logger.log("MCC https started: " + scope.config.ssl.options.address + ":" + scope.config.ssl.options.port);
