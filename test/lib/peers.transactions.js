...
describe("POST /peer/transactions", function () {
    ...
    it("Using valid transaction with wrong magic in headers. Should fail", function (done) {
        var transaction = node.mcc.transaction.createTransaction("1", 1, node.Gaccount.password);
    ...
    it("Using transaction with undefined recipientId. Should fail", function (done) {
        var transaction = node.mcc.transaction.createTransaction(undefined, 1, node.Gaccount.password);
        node.peer.post("/transactions")
            .set("Accept", "application/json")
            .set("version",node.version)
            .set("magic", node.config.magic)
    ...
    it("Using invalid passphrase. Should fail", function (done) {
        var transaction = node.mcc.transaction.createTransaction("12", 1, node.Gaccount.password);
        transaction.recipientId = "1";
        transaction.id = node.mcc.crypto.getId(transaction);
        node.peer.post("/transactions")
            .set("Accept", "application/json")
            .set("version",node.version)
            .set("magic", node.config.magic)
    ...
    it("When sender has no funds. Should fail", function (done) {
        var transaction = node.mcc.transaction.createTransaction("1", 1, "randomstring");
        node.peer.post("/transactions")
            .set("Accept", "application/json")
            .set("version",node.version)
            .set("magic", node.config.magic)
    ...
    it("Using very large amount and genesis block id. Should fail", function (done) {
        var transaction = node.mcc.transaction.createTransaction("12", 10000000000000000, node.Gaccount.password);
    ...
    it("Using overflown amount. Should fail", function (done) {
        var transaction = node.mcc.transaction.createTransaction("12", 184819291270000000012910218291201281920128129, node.Gaccount.password);
        node.peer.post("/transactions")
            .set("Accept", "application/json")
            .set("version",node.version)
            .set("magic", node.config.magic)
    ...
    it("Using float amount. Should fail", function (done) {
        var transaction = node.mcc.transaction.createTransaction("12", 1.3, node.Gaccount.password);
        node.peer.post("/transactions")
            .set("Accept", "application/json")
            .set("version",node.version)
            .set("magic", node.config.magic)
