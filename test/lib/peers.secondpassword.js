...
describe("POST /peer/transactions", function () {
    ...
    describe("Enabling second passphrase", function () {
        ...
        it("When accounts has no funds. Should fail", function (done) {
            var transaction = node.mcc.signature.createSignature(node.randomPassword(), node.randomPassword());
                    ...
                    node.api.put("/transactions")
                        ...
                        .end(function (err, res) {
                            ...
                            node.onNewBlock(function (err) {
                                ...
                                var transaction = node.mcc.signature.createSignature(account.password, account.secondPassword);
...
describe("POST /peer/transactions", function () {
    ...
    describe("Sending normal transaction with second passphrase now enabled", function () {
        ...
        it("When account doesn't have a second passphrase. Should fail", function (done) {
            var transaction = node.mcc.transaction.createTransaction("1", 1, node.Gaccount.password, account.secondPassword);
            node.peer.post("/transactions")
                .set("Accept", "application/json")
                .set("version",node.version)
                .set("magic", node.config.magic)
        ...
        it("Using blank second signature. Should fail", function (done) {
            var transaction = node.mcc.transaction.createTransaction("1", 1, account.password, ""); // Send 1 MCC to address 1L
            node.peer.post("/transactions")
                .set("Accept", "application/json")
                .set("version", node.version)
                .set("magic", node.config.magic)
        ...
        it("Using fake second signature. Should fail", function (done) {
            var transaction = node.mcc.transaction.createTransaction("1", 1, account.password, account2.secondPassword); // Send 1 MCC to address 1L
        ...
        it("Using valid second signature. Should be ok", function (done) {
            var transaction = node.mcc.transaction.createTransaction("1", 1, account.password, account.secondPassword); // Send 1 MCC to address 1L
            node.peer.post("/transactions")
                .set("Accept", "application/json")
                .set("version", node.version)
                .set("magic", node.config.magic)
